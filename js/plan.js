...
export function printSelectedPlan(){
  ...
  // даже при минимально допустимых размерах не помещается — спрашиваем пользователя
  pending={html,orientation,fit};
  let pct=Math.round((fit.overflow-1)*100);
  $("#planFitInfo").textContent=`План не помещается на одну страницу даже при минимально допустимых размерах (шрифт ${fit.level.font}pt, поля ${fit.level.pageMargin}мм). Содержимое превышает страницу примерно на ${pct}%. Дальнейшее уменьшение сделает документ трудночитаемым, поэтому выберите, как поступить.`;
